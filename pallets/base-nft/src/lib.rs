...
pub mod pallet {
	...
	#[derive(Default, Encode, Decode, Clone, Eq, PartialEq, RuntimeDebug, scale_info::TypeInfo)]
...
impl<T: Config> Pallet<T> {
	pub fn create_class(
		...
		let info =
			ClassInfo { metadata, total_issuance: Default::default(), owner: owner.clone(), data };
	...
	pub fn transfer(
		...
		Tokens::<T>::try_mutate(token.0, token.1, |token_info| -> DispatchResult {
			...
			ensure!(token_info_value.owners.contains(from),Error::<T>::NoPermission);
			TokensByOwner::<T>::try_mutate_exists(from,token,|sender_token|-> DispatchResult{
				...
				ensure!(sender_token_value.percent_owned >= percentage,
		...
		NextTokenId::<T>::try_mutate(class_id, |id| -> Result<T::TokenId, DispatchError> {
			...
			*id = id.checked_add(&One::one()).ok_or(Error::<T>::NoAvailableTokenId)?;
			...
			Classes::<T>::try_mutate(class_id, |class_info| -> DispatchResult {
				...
				info.total_issuance =
					info.total_issuance.checked_add(&One::one()).ok_or(Error::<T>::NumOverflow)?;

				Ok(())
			})?;

			let token_info = TokenInfo { metadata, owners: [owner.clone()].to_vec(), data };
			...
			match TryInto::<u32>::try_into(token_id) {
				...
				Ok(token) => {
					...
					TokensByOwner::<T>::insert(
						owner,
						(class_id, token_id),
						TokenByOwnerData { percent_owned: 100, token_id: token },
					);
					Ok(token_id)
				},
			}
